...
def not_found(error) -> Tuple[jsonify, int]:
    ...
    This function is a Flask error handler that returns a JSON response
    with a 404 status code and a message "Not found". It is triggered
    when a 404 error is raised.

    Args:
        error (Exception): The error raised.

    Returns:
        Tuple[jsonify, int]: A JSON response with a 404 status code.
    """
...
def unauthorized(error: Exception) -> Tuple[jsonify, int]:
    ...
    This function is a Flask error handler that returns a JSON response
    with a 401 status code and a message "Unauthorized". It is triggered
    when a 401 error is raised.

    Args:
        error (Exception): The error raised.

    Returns:
        Tuple[jsonify, int]: A JSON response with a 401 status code.
    """
